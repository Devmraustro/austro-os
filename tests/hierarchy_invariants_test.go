...
func TestOrganizationHierarchyInvariants(t *testing.T) {
	...
	validDept := uuid.New()
	_, err = admin.Exec(`INSERT INTO departments (id, name, workspace_id) VALUES ($1,'Dept OK',$2)`, validDept, ws)
	require.NoError(t, err)
	validTeam := uuid.New()
	_, err = admin.Exec(`INSERT INTO teams (id, name, department_id) VALUES ($1,'Team OK',$2)`, validTeam, validDept)
	require.NoError(t, err)
...
func TestWorkspaceIsolationEnforcement(t *testing.T) {
	...
	teamA := uuid.New()
	empA := uuid.New()
	_, err = admin.Exec(`INSERT INTO departments (id, name, workspace_id) VALUES ($1,'A',$2)`, deptA, wsA)
	require.NoError(t, err)
	_, err = admin.Exec(`INSERT INTO teams (id, name, department_id) VALUES ($1,'A-T',$2)`, teamA, deptA)
	require.NoError(t, err)
	_, err = admin.Exec(`INSERT INTO ai_employees (id, name, role, team_id) VALUES ($1,'A-E','r',$2)`, empA, teamA)
	require.NoError(t, err)

	// Also create an unrelated workspace B department to prove B is separate.
	_, err = admin.Exec(`INSERT INTO departments (id, name, workspace_id) VALUES ($1,'B',$2)`, uuid.New(), wsB)
