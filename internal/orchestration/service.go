...
func NewService(store PipelineStore, research Researcher, script ScriptWriter, review Reviewer, publish Publisher, audit AuditSink, events EventSink) *Service {
	if audit == nil { audit = NullAuditSink{} }
	if events == nil { events = NullEventSink{} }
	if research == nil { research = StubResearcher{} }
	if script == nil { script = StubScriptWriter{} }
	if review == nil { review = StubReviewer{} }
	if publish == nil { publish = StubPublisher{} }
...
func (s *Service) CreateWithIdempotency(ctx context.Context, workspaceID uuid.UUID, goalID *uuid.UUID, traceID, key string) (*Pipeline, error) {
	if workspaceID == uuid.Nil { return nil, ErrWorkspaceMismatch }
	key = strings.TrimSpace(key)
	if len(key) > 128 { return nil, ErrInvalidInput }
	if key != "" {
		if finder, ok := s.store.(interface { GetByIdempotency(context.Context, uuid.UUID, string) (*Pipeline, error) }); ok {
			if existing, err := finder.GetByIdempotency(ctx, workspaceID, key); err == nil {
				if existing.Stage == StageResearch && existing.Status == StatusCreated {
					if publishErr := s.events.PublishPipeline(ctx, "pipeline.research", existing.ID, workspaceID, string(existing.Stage), existing.TraceID, spanOf(ctx)); publishErr != nil { return existing, publishErr }
				}
				return existing, nil
...
func (s *Service) Advance(ctx context.Context, workspaceID, id uuid.UUID, startStage, to Stage) (*Pipeline, error) {
	if workspaceID == uuid.Nil { return nil, ErrWorkspaceMismatch }
	if !s.throttle.Allow(workspaceID.String()) { return nil, ErrRateLimited }
	p, err := s.getOwned(ctx, workspaceID, id)
	if err != nil { return nil, err }
	if p.Stage != startStage { return nil, ErrConsecutiveAdvance }
	if err := CanAdvance(to, p.Stage, p.Status); err != nil { return nil, err }
...
func (s *Service) Approve(ctx context.Context, workspaceID, id uuid.UUID, actor string) (*Pipeline, error) {
	...
	if actor == "" { return nil, ErrUnauthorizedActor }
	p, err := s.getOwned(ctx, workspaceID, id)
	if err != nil { return nil, err }
	if p.Stage != StageReview || p.Status != StatusAwaitingApproval { return nil, ErrApprovalRequired }
	if s.publication != nil {
		if p.PublicationID == nil { return nil, ErrApprovalRequired }
		if err := s.publication.Approve(ctx, workspaceID, *p.PublicationID, actor); err != nil { return nil, err }
	}
	now := time.Now().UTC()
	...
	if err := s.events.PublishPipeline(ctx, "pipeline.review_approved", id, workspaceID, string(StageReview), traceOf(ctx), spanOf(ctx)); err != nil { return updated, err }
	return updated, nil
}
...
func (s *Service) Retry(ctx context.Context, workspaceID, id uuid.UUID, actor string) (*Pipeline, error) {
	...
	if p.Stage == StageComplete { return nil, ErrTerminalState }
	switch p.Stage {
	case StageReview:
		if p.ApprovedBy != "" && p.ApprovedAt != nil { p.Status = StatusApproved } else { p.Status = StatusAwaitingApproval }
	case StageResearch:
	...
	if _, err := s.store.Update(ctx, workspaceID, p); err != nil { return nil, err }
...
func (s *Service) doAdvance(ctx context.Context, p *Pipeline, to Stage, actorType, actorID string) (*Pipeline, error) {
	...
	if err != nil {
		...
		if persistErr != nil { return nil, errors.Join(err, persistErr) }
		return nil, err
	}
	...
	if to == StagePublish && s.publication != nil {
		if p.PublicationID == nil || !p.Approved() { return nil, ErrApprovalRequired }
	...
	p.Stage = to
	switch to {
	case StageScript, StagePublish: p.Status = StatusActive
	case StageReview: p.Status = StatusAwaitingApproval
	case StageComplete: p.Status = StatusDone
	}
	p.FailureReason = ""
	p.UpdatedAt = time.Now().UTC()
	...
	if to == StageReview { eventType = "pipeline.review_ready" }
	if err := s.events.PublishPipeline(ctx, eventType, p.ID, p.WorkspaceID, string(to), traceOf(ctx), spanOf(ctx)); err != nil { return updated, err }
...
func (s *Service) stageWork(ctx context.Context, p *Pipeline, to Stage) (string, error) {
	...
	case StageReview:
		...
		if err != nil { return "", err }
		if !ok { return "", errors.New("orchestration: review rejected artefact") }
		p.ReviewReference = "review-ok"
		return p.ReviewReference, nil
	case StagePublish:
		if s.publication == nil { return s.publish.Publish(ctx, p.WorkspaceID, p, p.ReviewReference) }
		return "", nil
	case StageComplete:
		return "", nil
	default:
...
func boundReference(ref string) string {
	...
	if len(ref) > 512 { return ref[:512] }
	return ref
}

func failureReason(err error) string {
	if err == nil { return "stage_failed" }
	msg := strings.TrimSpace(err.Error())
	if len(msg) > 512 { return msg[:512] }
	if msg == "" { return "stage_failed" }
	return msg
}

func (s *Service) Get(ctx context.Context, workspaceID, id uuid.UUID) (*Pipeline, error) { return s.getOwned(ctx, workspaceID, id) }
func (s *Service) List(ctx context.Context, workspaceID uuid.UUID) ([]*Pipeline, error) { return s.store.List(ctx, workspaceID) }

func (s *Service) getOwned(ctx context.Context, workspaceID, id uuid.UUID) (*Pipeline, error) {
	if workspaceID == uuid.Nil || id == uuid.Nil { return nil, ErrInvalidInput }
	p, err := s.store.Get(ctx, workspaceID, id)
	if err != nil { return nil, err }
	if p.WorkspaceID != workspaceID { return nil, ErrWorkspaceMismatch }
	return p, nil
}

func actorID(ctx context.Context) string { if v, ok := ctx.Value(actorKey{}).(string); ok { return v }; return "" }
func actorType(ctx context.Context, fallback string) string { if actorID(ctx) != "" { return "human" }; return fallback }
func traceOf(ctx context.Context) string { if v, ok := ctx.Value(traceKey{}).(string); ok { return v }; return "" }
func spanOf(ctx context.Context) string { if v, ok := ctx.Value(spanKey{}).(string); ok { return v }; return "" }
...
func WithActor(ctx context.Context, id string) context.Context { return context.WithValue(ctx, actorKey{}, strings.TrimSpace(id)) }

func logTrace(workspaceID uuid.UUID, msg string) *logger.Entry { return logger.NewEntry(msg).With("workspace_id", workspaceID) }
